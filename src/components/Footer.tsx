      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-10 mb-12">
          <div className="md:col-span-1">
            <div className="text-lg font-bold mb-1">The Clean Air Trees Project</div>
            <p className="text-white/40 text-xs leading-relaxed mt-3">
              Planting trees. Restoring nature. Protecting our future — one community at a time.
            </p>
          </div>
          {Object.entries(cols).map(([title, items]) => (
            <div key={title}>
              <h4 className="text-[11px] text-white/35 uppercase tracking-widest mb-4">{title}</h4>
              <ul className="flex flex-col gap-2.5 list-none">
                {items.map((i) => (
                  <li key={i.label}>
                    <Link href={i.href} className="text-[13px] text-white/55 hover:text-forest-pale transition-colors no-underline">
                      {i.label}
                    </Link>
                  </li>
                ))}
